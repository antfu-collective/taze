...
cli
  .command('[mode]', `Update mode (version range to check). Available: ${MODE_CHOICES.join(' | ')}`, { ignoreOptionDefaultValue: true })
  ...
  .option('--recursive, -r', 'recursively search supported files in subdirectories')
  ...
  .option('--ignore-paths <paths>', 'ignore paths when searching supported files')
  .option('--ignore-other-workspaces', 'ignore files in other workspaces (with their own .git,pnpm-workspace.yaml,etc.)', { default: true })
  .option('--no-github-actions', 'disable checking GitHub Actions in .github/workflows and composite action.yml files')
  .option('--no-node-version', 'disable checking .node-version files')
