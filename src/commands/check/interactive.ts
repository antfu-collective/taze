  ...
  pkgs.forEach((pkg) => {
    pkg.resolved.forEach((dep) => {
      if (dep.update && !dep.provenanceDowngraded) {
        ...
        dep.update = true
        updateTargetVersion(dep, dep.latestVersionAvailable, undefined, options.includeLocked, options.lockedUpgradeMode)
      }
    })
  })
    ...
    return {
      ...
      onKey(key) {
        switch (key.name) {
          ...
          case 'h':
          case 'l':
            updateTargetVersion(dep, versions[index].version, undefined, options.includeLocked, options.lockedUpgradeMode)
